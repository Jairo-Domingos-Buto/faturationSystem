            ...
            <div
                ...
                <div>
                    ...
                    <p class="text-sm text-orange-700">
                        Alterando documento original: <strong
                            class="font-mono bg-orange-200 px-2 rounded">{{ $documentoOriginalNumero }}</strong>
                    </p>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 space-y-5">
                        ...
                        <div class="p-3 bg-blue-50 border border-blue-100 rounded-lg flex items-center gap-3 cursor-pointer hover:bg-blue-100 transition-colors"
                            ...
                            <div class="overflow-hidden">
                                ...
                                <p class="text-xs text-slate-500">
                                    {{ $clienteSelecionado ? 'Cliente Registado' : 'Consumidor Final' }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col h-[500px]">
                    <!-- Barra Unificada de Busca + Natureza -->
                    <div class="p-4 ">
                        <div
                            class="flex  items-center bg-slate-50 border border-slate-300 rounded-xl transition-all shadow-sm overflow-hidden">
                            ...
                            <input type="search" wire:model.live.debounce.300ms="searchProdutoTerm"
                                class="flex-1 bg-transparent py-3 text-sm placeholder-slate-400 focus:outline-none"
                                placeholder="{{ $natureza === 'produto'
                                    ? 'Pesquisar produto por nome ou código de barras...'
                                    : 'Pesquisar serviço por descrição...' }}" />

                            <!-- Limpar -->
                            @if ($searchProdutoTerm)
                                <button wire:click="limparBusca"
                            ...
                            <div class="flex items-center gap-1 pr-2">
                                ...
                                <!-- Produto -->
                                <label
                                    class="flex items-center gap-1 px-3 py-2 rounded-lg cursor-pointer text-sm font-medium transition-all
                {{ $natureza === 'produto'
                    ? 'bg-blue-600 text-white shadow ring-2 ring-blue-300'
                    : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                    <input type="radio" class="hidden" wire:click="alterarNatureza('produto')"
                                        {{ $natureza === 'produto' ? 'checked' : '' }}>
                ...
                {{ $natureza === 'servico'
                    ...
                    : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                    <input type="radio" class="hidden" wire:click="alterarNatureza('servico')"
                                        {{ $natureza === 'servico' ? 'checked' : '' }}>
                        ...
                        <p class="text-[10px] text-slate-400 mt-2 text-center">
                            @if ($natureza === 'produto')
                                Exibindo stock e código de barras
                            @else
                                Serviço isento de stock e impostos (M02)
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="flex flex-1 overflow-hidden">
                        ...
                        <div
                            ...
                            <h4
                                ...
                                <span>{{ $natureza === 'produto' ? 'Produtos Disponíveis' : 'Serviços Disponíveis' }}</span>
                                            ...
                                            <button wire:click="adicionarProduto({{ $produto->id }})"
                                                ...
                                                <div class="relative z-10">
                                                    <div
                                                        ...
                                                        <i class='bx bx-package text-blue-500 mt-0.5'></i>
                                                        <!-- ÍCONE DE PRODUTO -->
                                                        {{ $produto->descricao }}
                                                    </div>
                                                    ...
                                                    <div class="flex justify-between items-end mt-2">
                                                        ...
                                                        <span
                                                            class="text-lg font-bold text-slate-800">{{ number_format($produto->preco_venda, 2, ',', '.') }}</span>
                                                    </div>
                                                </div>
                                            </button>
                                        @endforeach
                                            ...
                                            <button wire:click="adicionarServico({{ $servico->id }})"
                                                ...
                                                <div class="relative z-10">
                                                    ...
                                                    <div class="flex justify-between items-center mt-auto">
                                                        ...
                                                        <span
                                                            class="text-lg font-bold text-slate-800">{{ number_format($servico->valor ?? $servico->preco_venda, 2, ',', '.') }}</span>
                                                    </div>
                                                </div>
                                            </button>
                                        @endforeach
                        ...
                        <div class="w-2/5 flex flex-col bg-white">
                            <div class="p-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                                ...
                                <span
                                    class="bg-blue-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ count($produtosCarrinho) }}</span>
                            </div>
                            ...
                            @if (count($produtosCarrinho) > 0)
                                <div
                                    ...
                                    <span>Qtd Total:
                                        <strong>{{ array_sum(array_column($produtosCarrinho, 'quantidade')) }}</strong></span>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl shadow-lg overflow-hidden sticky top-6">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div
                            ...
                            <span
                                class="text-3xl font-extrabold text-blue-700 tracking-tight">{{ number_format($total, 2, ',', '.') }}<span
                                    class="text-sm font-bold ml-1 opacity-70">KZ</span></span>
                        </div>
                            ...
                            <span wire:loading.remove class="flex items-center justify-center gap-2">
                                ...
                                @else
                                    @switch($tipoDocumento)
                                        @case('FR')
                                            <i class='bx bx-check-double text-2xl'></i> Emitir Fatura-Recibo
                                        @break

                                        @case('FT')
                                            <i class='bx bx-file text-2xl'></i> Emitir Fatura
                                        @break

                                        @case('FP')
                                            <i class='bx bx-time-five text-2xl'></i> Criar Proforma
                                        @break

                                        @case('RC')
                                            <i class='bx bx-receipt text-2xl'></i> Gerar Recibo
                                        @break

                                        @default
                                            <i class='bx bx-check text-2xl'></i> Finalizar
                                    @endswitch
                                @endif
                            </span>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[60] transition-all p-4"
            ...
            <div
                ...
                <!-- Modal Header -->
                <div
                    class="flex justify-between items-center p-5 border-b border-slate-100 bg-white sticky top-0 z-10">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500 mt-0.5">
                                        <span class="flex items-center gap-1"><i class='bx bx-id-card'></i>
                                            {{ $cliente->nif }}</span>
                                        @if ($cliente->telefone)
                                            <span class="flex items-center gap-1"><i class='bx bx-phone'></i>
